  ...
  return (
    <>
      <div>
        ...
        <p className='text-sm text-gray-600'>Get Started with the personal infromation</p>
        <div className='flex items-center justify-between gap-2'>
          ...
          {typeof data.image === 'object' && (
            <div className='flex flex-col gap-1 pl-4 text-sm'>
              ...
              <label className='realtive inline-flex items-center cursor-pointer text-gray-900 gap-3'>
            ...
            return(
              <div key={field.key} className='space-y-1 mt-5'>
                <label className='flex items-center gap-2 text-sm font-medium text-gray-600'>
                  ...
                  {React.createElement(field.icon, { className: 'size-4' })}
                  {field.label}
                  {field.required && <span className='text-red-500'>*</span> }
                </label>
                <input type={field.type} value={data[field.key] || ""}
                onChange={(e)=>handleChange(field.key, e.target.value)} className='mt-1 w-full
                px-3 py-2 border border-gray-300 rounded-lg focus:ring focus:ring-blue-500
